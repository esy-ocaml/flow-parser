...
let () =
  Printf.printf
    {sexp|
(library
 ((name flow-parser.ppx_tools_versioned)
  (public_name flow-parser.ppx_tools_versioned)
...
let () =
  List.iter (fun version ->
      Printf.printf
        {sexp|
(library
 ((name flow-parser.ppx_tools_versioned_metaquot_%s)
  (public_name flow-parser.ppx_tools_versioned.metaquot_%s)
  (synopsis "Meta-quotation: %s parsetree quotation")
  (libraries (ocaml-migrate-parsetree flow-parser.ppx_tools_versioned))
  (kind ppx_rewriter)
  (ppx_runtime_libraries (flow-parser.ppx_tools_versioned))
  (wrapped false)
